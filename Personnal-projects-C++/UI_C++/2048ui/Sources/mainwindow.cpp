...
//static int number_record = 2; // 我以为把纪录设置为静态变量就可以直接保存修改了，too naive。。。
...
void MainWindow::update_view(){
    choosetohide(ui->lcdNumber00, mygame.show_map(0, 0));
    choosetohide(ui->lcdNumber01, mygame.show_map(0, 1));
    choosetohide(ui->lcdNumber02, mygame.show_map(0, 2));
    choosetohide(ui->lcdNumber03, mygame.show_map(0, 3));

    choosetohide(ui->lcdNumber10, mygame.show_map(1, 0));
    choosetohide(ui->lcdNumber11, mygame.show_map(1, 1));
    choosetohide(ui->lcdNumber12, mygame.show_map(1, 2));
    choosetohide(ui->lcdNumber13, mygame.show_map(1, 3));

    choosetohide(ui->lcdNumber20, mygame.show_map(2, 0));
    choosetohide(ui->lcdNumber21, mygame.show_map(2, 1));
    choosetohide(ui->lcdNumber22, mygame.show_map(2, 2));
    choosetohide(ui->lcdNumber23, mygame.show_map(2, 3));

    choosetohide(ui->lcdNumber30, mygame.show_map(3, 0));
    choosetohide(ui->lcdNumber31, mygame.show_map(3, 1));
    choosetohide(ui->lcdNumber32, mygame.show_map(3, 2));
    choosetohide(ui->lcdNumber33, mygame.show_map(3, 3));
...
void MainWindow::keyPressEvent(QKeyEvent *qke){
    if (game_end)
        return;
    while (game_begin == 0){
        if (qke->key() == Qt::Key_Up || qke->key() == Qt::Key_Down || qke->key() == Qt::Key_Left || qke->key() == Qt::Key_Right){
            game_begin = 1;
            mycount.start();
        }
        else
    ...
    else { // 设置一个permit，这样输完record后就不能用键盘控制了，要重新开始游戏才能把permit重新设为1}
        switch(qke->key()){
            case Qt::Key_Up:
                if (break_flag == 1){
                    break_flag = 0;
...
void MainWindow::on_Isover(){
    ...
    else
    {
        if (my_number_record <= number_record || my_time_record >= time_record){
            ...
            connect(new_game_pbt, SIGNAL(clicked(bool)),&overdialog, SLOT(reject()));
            connect(set_record_pbt, SIGNAL(clicked(bool)), &overdialog, SLOT(accept()));
...
// 注意这里不需要再加上一个exec()， 因为如果再写一次的话，那么如果选了setrecord就相当于到else那个分支又执行了一次exec，所以之前要点两次setrecord
...
void MainWindow::on_about_game(){
    QString introduction(
        "<h2>" + tr("About 2048") + "</h2>"
        + "<p>" + tr("This game is played by using the direction keys to move the numbers, the numbers on the moving direction will be accumulated automatiquely if they are the same. During the game you can tap space key to take a break and you can tap again or use direction keys to get back in the game. There are two records, one is the time minimal to reach 2048 and the other one is the biggest number you have ever reached. Enjoy your game!")+"</p>"
...
//    messageBox.setWindowIcon(QIcon("2048.png"));
